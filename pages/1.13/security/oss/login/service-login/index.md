...
A service logs in to DC/OS cryptographically secure without transmitting a password by supplying a service login token.
...
**NOTE**: Service login tokens and DC/OS Authentications are both JWTs of type RS256 with identical claims. However they are not interchangable. A service login token is signed by a service private key while a DC/OS Authentication token is signed by the IAM private key.</p>
