...
User accounts can be managed by any authorized user. To bootstrap this process, in DC/OS an external user is automatically created by the first user who logs in via the web interface.

<p class="message--note"><strong>NOTE: </strong>Any user with access to DC/OS can create more user accounts. Each user account in DC/OS is an authorized administrator account, there is no explicit concept of privileges with DC/OS.</p>
